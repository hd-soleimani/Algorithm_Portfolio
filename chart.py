...
main_excel = xlrd.open_workbook('Results_OPT_211104.xls')
...
for i in sheet_array:
    ...
    result_opt='Results_OPT_211104.xls'
    result_topk='Results_TopN_211103.xls'
    result_psfs='Results_PSFS_211104.xls'
    result_rnd='Results_RND_211104.xls'
    result_icarus='Results_ICARUS_211104.xls'
    ...
    CV_mean_topk=df_result_topk['CV_score_mean (1 is the best)']
    CV_mean_topk=np.log10(CV_mean_topk)

    CV_mean_psfs=df_result_psfs.loc[df_result_psfs['Epsilon']==epsilon,"CV_prediction_mean"]
    CV_mean_psfs=np.log10(CV_mean_psfs)

    CV_mean_rnd=df_result_rnd.loc[df_result_rnd['Epsilon']==epsilon,"CV_prediction_mean"]
    CV_mean_rnd=np.log10(CV_mean_rnd)

    CV_mean_icarus=df_result_icarus.loc[df_result_icarus['Epsilon']==epsilon,"CV_prediction_mean"]
    ...
    plt.plot(X, Yc,linestyle='-.',label=("PSFS, e="+str(epsilon)))# dashdot
    plt.plot(X, Yd,linestyle=':',label=("RND, e="+str(epsilon)))# dotted
    ...
    plt.plot(X, Yh,'bo',label="ICARUS, e=0.5") 
    ...
    plt.plot(X, Yk,'bs',label="ICARUS, e=2") 
    ...
    plt.title(i)
    #plt.savefig(i+'.eps')
    #plt.savefig(i+'_e'+str(epsilon)+'.eps', bbox_extra_artists=(lgd,), bbox_inches='tight') #legend show in save fig
    plt.savefig(i+'_eselected'+'.eps', bbox_extra_artists=(lgd,), bbox_inches='tight') #legend show in save fig
